...
class Client(BaseClient):
    def __init__(
        ...
        self.max_indicator_relationships = (
            0 if not max_indicator_relationships else max_indicator_relationships
        )
    ...
    def query(
        ...
        if sub_section == "passive_dns":
            return self._http_request(
                "GET", url_suffix=suffix, params=params, timeout=30
            )
        try:
            ...
            if hasattr(e.res, "status_code"):
                if e.res.status_code == 404:
                    ...
                    demisto.debug(
                        f"{e.res.text} response received from server when trying to get api:{e.res.url}"
                    )
                    raise Exception(
                        f"The command could not be execute: {argument} is invalid."
                    )
                else:
                    raise
            else:
...
def create_list_by_ec(list_entries: list, list_type: str) -> list:
    def create_entry_by_ec(entry: dict) -> dict:
        ...
        if list_type == "url_list":
            return assign_params(**{"Data": entry.get("url")})

        if list_type == "hash_list":
            return assign_params(**{"Hash": entry.get("hash")})
...
def relationships_manager(
    ...
    attack_ids: list,
    indicator: str,
    field_for_passive_dns_rs: str,
    feed_indicator_type_for_passive_dns_rs: str,
):
    """
    ...
    if client.max_indicator_relationships > 0:
        ...
        _, _, urls_raw_response = alienvault_get_related_urls_by_indicator_command(
            client, indicator_type, indicator, limit
        )
        urls_raw_response = delete_duplicated_entities(
            urls_raw_response.get("url_list", []), "url"
        )
        relationships += create_relationships(
            client,
            urls_raw_response,
        ...
        _, _, hash_raw_response = alienvault_get_related_hashes_by_indicator_command(
            client, indicator_type, indicator, limit
        )
        hash_raw_response = delete_duplicated_entities(
            hash_raw_response.get("data", []), "hash"
        )
        relationships += create_relationships(
            client,
            hash_raw_response,
        ...
        passive_dns_raw_response = delete_duplicated_entities(
            passive_dns_raw_response.get("passive_dns", []), field_for_passive_dns_rs
        )
        passive_dns_raw_response = validate_string_is_not_url(
            passive_dns_raw_response, field_for_passive_dns_rs
        )
        passive_dns_raw_response = passive_dns_raw_response[
...
def create_relationships(
    ...
    if (
        relevant_field
        and isinstance(relevant_field, list)
        and relevant_id in relevant_field[0]
    ):
...
def delete_duplicated_entities(entities_list: List[Dict], field_name: str):
    ...
    for entity_dict in entities_list:
        if (
            ...
            and (ind_value := entity_dict.get(field_name)) not in unique_dict.keys()
        ):
...
def validate_string_is_not_url(entities_list: List[Dict], field_name: str):
    ...
    return [
        ...
        if not auto_detect_indicator_type(dict.get(field_name)) == "URL"
    ]
...
@logger
def ip_command(
    client: Client, ip_address: str, ip_version: str
) -> List[CommandResults]:
    ...
    for ip_ in ips_list:
        ...
        if raw_response and raw_response != 404:
            ip_version = (
                FeedIndicatorType.IP if ip_version == "IPv4" else FeedIndicatorType.IPv6
            )
            relationships = relationships_manager(
                client,
                entity_a=ip_,
                entity_a_type=ip_version,
            ...
            context = {"Reputation": raw_response.get("reputation"), "IP": ip_}

            human_readable_context = {
                "Address": ip_object.to_context()
                .get("IP(val.Address && val.Address == obj.Address)")
                .get("Address"),
                "Geo": ip_object.to_context()
                .get("IP(val.Address && val.Address == obj.Address)")
                .get("Geo"),
            }

            human_readable = tableToMarkdown(name=title, t=human_readable_context)
    ...
    if not command_results:
        return [
            CommandResults(
                f"{INTEGRATION_NAME} - Could not find any results for given query."
            )
        ]
    ...
    for domain in domains_list:
        ...
        if raw_response and raw_response != 404:
            ...
            domain_object = Common.Domain(
                domain=domain, dbot_score=dbot_score, relationships=relationships
            )
...
def file_command(client: Client, file: str) -> List[CommandResults]:
    ...
    for hash_ in hashes_list:
        raw_response_analysis = client.query(
            section="file", argument=hash_, sub_section="analysis"
        )
        raw_response_general = client.query(section="file", argument=hash_)
        if (
            raw_response_analysis
            and raw_response_general
            and (
                shortcut := dict_safe_get(
                    raw_response_analysis, ["analysis", "info", "results"], {}
                )
            )
            and raw_response_general != 404
            and raw_response_analysis != 404
        ):
            relationships = create_relationships(
                client,
                extract_attack_ids(raw_response_general),
                hash_,
                FeedIndicatorType.File,
                "display_name",
            ...
            dbot_score = Common.DBotScore(
                ...
                malicious_description=raw_response_general.get("pulse_info", {}).get(
                    "pulses"
                ),
                reliability=client.reliability,
            )
            ...
            file_object = Common.File(
                ...
                file_type=shortcut.get("file_type"),
                dbot_score=dbot_score,
                relationships=relationships,
            )
        ...
        if raw_response:
            if raw_response == 404:
                ...
                if client.create_relationships:
                    indicator = FeedIndicatorType.indicator_type_by_server_version(
                        "STIX Attack Pattern"
                    )
                    relationships = create_relationships(
                        client,
                        extract_attack_ids(raw_response),
                        url,
                        FeedIndicatorType.URL,
                        "display_name",
                        indicator,
                ...
                url_object = Common.URL(
                    url=url, dbot_score=dbot_score, relationships=relationships
                )
...
def alienvault_search_cve_command(
    ...
    Returns:
        Outputs
    """
    raw_response = client.query(section="cve", argument=cve_id)
    if raw_response and raw_response != 404:
        title = f"{INTEGRATION_NAME} - Results for Hostname query"
        context_entry: dict = {
            outputPaths.get("cve"): {
        ...
        human_readable = tableToMarkdown(
            t=context_entry.get(outputPaths.get("cve")), name=title
        )
...
def alienvault_get_related_urls_by_indicator_command(
    client: Client, indicator_type: str, indicator: str, limit: str = ""
) -> Tuple[str, Dict, Dict]:
    ...
    if raw_response and raw_response != 404:
        ...
        context_entry: list = create_list_by_ec(
            list_entries=raw_response.get("url_list", {}), list_type="url_list"
        )
        context: dict = {
...
def alienvault_get_related_hashes_by_indicator_command(
    ...
    if limit:
        params["limit"] = limit
    raw_response = client.query(
        section=indicator_type, argument=indicator, sub_section="malware", params=params
    )
    if raw_response and raw_response != 404:
        title = f"{INTEGRATION_NAME} - Related malware list to queried indicator"
        context_entry: dict = {
            "AlienVaultOTX.File(val.File.Hash && val.File.Hash == obj.File.Hash)": create_list_by_ec(
                list_entries=raw_response.get("data", {}), list_type="hash_list"
            )
        }
        human_readable = tableToMarkdown(
            t=context_entry.get(
                "AlienVaultOTX.File(val.File.Hash && val.File.Hash \
                                            == obj.File.Hash)"
            ),
            name=title,
        )
...
def alienvault_get_passive_dns_data_by_indicator_command(
    ...
    if raw_response and raw_response != 404:
        ...
        context_entry: dict = {
            ...
            "val.PassiveDNS.IP && val.PassiveDNS.IP == obj.PassiveDNS.IP)": create_list_by_ec(
                list_entries=raw_response.get("passive_dns", {}),
...
def alienvault_search_pulses_command(
    ...
    """
    raw_response = client.query(
        section="search", sub_section="pulses", params={"page": page}
    )
    if raw_response and raw_response != 404:
...
def alienvault_get_pulse_details_command(
    ...
    Returns:
        Outputs
    """
    raw_response = client.query(section="pulses", argument=pulse_id)
    if raw_response and raw_response != 404:
        ...
        context_entry: dict = {
            "AlienVaultOTX.Pulses(val.ID && val.ID == obj.ID)": {
                ...
                "Author": {"Username": raw_response.get("author", {}).get("username")},
        ...
        human_readable = tableToMarkdown(
            t=context_entry.get("AlienVaultOTX.Pulses(val.ID && val.ID == obj.ID)"),
            name=title,
        )
...
def main():
    ...
    max_indicator_relationships = arg_to_number(
        params.get("max_indicator_relationships", 0)
    )
    token = params.get("credentials", {}).get("password", "") or params.get(
        "api_token", ""
    )
